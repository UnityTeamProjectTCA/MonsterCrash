...
namespace BreadcrumbAi {
	...
	public class Ai : MonoBehaviour {
		...
		public float followSpeed, wanderSpeed, patrolSpeed, rotationSpeed, avoidSpeed,
					 ...
					 followDistance, wanderDistance, attackDistance, visionDistance, avoidPlayerDistance, avoidBuildingDistance, edgeDistance, otherAiDistance,
		...
		void FixedUpdate ( ) {
			...
			Ai_Avoidance( ~( breadcrumbLayer | enemyLayer | waypointLayer | playerLayer | floorLayer ) );    // Controls Ai wall avoidance
			Ai_Hover( );
		}
		...
		private void Ai_Avoidance ( LayerMask Layer ) {
			...
			if ( _HasAvoidance ) {
				if ( moveState == MOVEMENT_STATE.IsFollowingPlayer ) {
					Vector3 look_pos = Vector3.Scale( player.transform.position, new Vector3( 1, 0, 1 ) ) + transform.up * transform.position.y;
					transform.LookAt( look_pos );
				}
				if ( Physics.Raycast( transform.position, transform.forward + -transform.right, out hit, avoidBuildingDistance, buildingLayer ) ) {
					Debug.DrawLine( transform.position, hit.point, Color.cyan );
					GetComponent<Rigidbody>( ).AddForce( transform.forward + transform.right * avoidSpeed );
					_IsAvoiding = true;
				}
				if ( Physics.Raycast( transform.position, transform.forward + transform.right, out hit, avoidBuildingDistance, buildingLayer ) ) {
					Debug.DrawLine( transform.position, hit.point, Color.cyan );
				...
				if ( Physics.Raycast( transform.position, -transform.forward, out hit, avoidBuildingDistance, buildingLayer ) ) {
					Debug.DrawLine( transform.position, hit.point, Color.cyan );
					GetComponent<Rigidbody>( ).AddForce( transform.forward * avoidSpeed );
					_IsAvoiding = true;
				}
				if ( Physics.Raycast( transform.position, -transform.right, out hit, avoidBuildingDistance, buildingLayer ) ) {
					Debug.DrawLine( transform.position, hit.point, Color.cyan );
				...
				if ( Physics.Raycast( transform.position, -transform.forward + -transform.right * 2, out hit, avoidBuildingDistance, buildingLayer ) ) {
					Debug.DrawLine( transform.position, hit.point, Color.cyan );
					GetComponent<Rigidbody>( ).AddForce( transform.right * avoidSpeed );
					_IsAvoiding = true;
				}
				...
				if ( Physics.CheckSphere( transform.position, avoidPlayerDistance, playerLayer ) ) {
					Vector3 check_pos = player.transform.position - transform.position;
					transform.LookAt( -check_pos );
					GetComponent<Rigidbody>( ).AddForce( -check_pos * avoidSpeed );
					_IsAvoiding = true;
				}
