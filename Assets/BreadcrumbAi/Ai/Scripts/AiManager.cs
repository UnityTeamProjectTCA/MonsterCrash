	...
	public static class AiManager {
        ...
        */
		...
		public static IEnumerator Ai_Layers ( this Ai ai ) {
			...
			ai.floorLayer = ( 1 << LayerMask.NameToLayer( floorString ) );
			ai.buildingLayer = ( 1 << LayerMask.NameToLayer( buildingString ) );
			yield return null;
		}
		...
		public static Transform Ai_FindPlayer ( this Ai ai ) {
			if ( ai.Player ) {
				if ( ai.InRange( ai.Player.position, ai.visionDistance ) ) {
					if ( Physics.Linecast( ai.transform.position, ai.Player.position, out hit, ~( ai.breadcrumbLayer | ai.waypointLayer | ai.buildingLayer ) ) ) {
